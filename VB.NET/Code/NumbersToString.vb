...
Module NumbersToString

    Friend Function GetNumbersToString(x As String, dram As Boolean) As String
        Dim a() As String
        Dim b As String
        Dim c As Integer
...
Dim id As Decimal = jAmount
Dim ix As Integer = (id Mod CLng(id \ id)) * 100

If ix > 0 Then
	sheet.Range("A15").Value = GetNumbersToString(CLng(jAmount), True) & " " & GetNumbersToString(CLng(ix), False) & " լումա"
	sheet.Range("A39").Value = GetNumbersToString(CLng(jAmount), True) & " " & GetNumbersToString(CLng(ix), False) & " լումա"
Else
	sheet.Range("A15").Value = GetNumbersToString(CLng(jAmount), True)
	sheet.Range("A39").Value = GetNumbersToString(CLng(jAmount), True)
End If